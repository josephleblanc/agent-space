...
//!
//! Exported functions are called from `web/js/game-bridge.js`.
...
/// Latest room snapshot applied from JS polling / canned mode.
...
pub fn drain_inbound_bridge(
    ...
    for json in inbound.room_states.drain(..) {
        match serde_json::from_str::<RoomSnapshot>(&json) {
            ...
            Err(error) => {
                log::warn!("room-state sync parse error: {error}");
            }
        }
    }

    for speech in inbound.speeches.drain(..) {
        log::info!("agent speech ({}): {}", speech.agent_id, speech.text);
...
fn push_room_state(json: String) {
    if let Ok(mut inbound) = INBOUND.get_or_init(Default::default).lock() {
        inbound.room_states.push(json);
    }
}

fn push_agent_speech(agent_id: String, text: String) {
    if let Ok(mut inbound) = INBOUND.get_or_init(Default::default).lock() {
        inbound.speeches.push(AgentSpeechInbound { agent_id, text });
    }
}

/// Apply a `RoomSnapshot` JSON payload from the JS poll loop or canned mode.
#[wasm_bindgen(js_name = on_room_state_sync)]
pub fn on_room_state_sync(json: &str) {
    push_room_state(json.to_owned());
}

/// Notify Bevy that an agent is speaking (Vapi TTS / agent-chat hook).
#[wasm_bindgen(js_name = on_agent_speech)]
pub fn on_agent_speech(agent_id: &str, text: &str) {
    push_agent_speech(agent_id.to_owned(), text.to_owned());
}

/// Convenience export for JS to verify the WASM bridge is alive.
